...
create_app_vm () {
    printf '%s\n'
    echo "###-------------------------------------###"
    echo Creating VM $VMNAME in RG $RGNAMEaz vm create --name $VMNAME --resource-group $RGNAME  --os-disk-name ${VMNAME}-osdisk --os-disk-size-gb 63 --storage-sku StandardSSD_LRS --size $VMTYPE --vnet-name $VNETNAME  --location $AZLOC --accelerated-networking true --public-ip-address '' --private-ip-address ${APPLSUBNET}.${ip} --image $VMIMAGE --admin-username=$ADMINUSR --ssh-key-value=$ADMINUSRSSH --subnet=${VNETNAME}-appl --nsg nsg-${AZLOCTLA}sap-${SIDLOWER}-app --zone $i --ppg ppg-${AZLOCTLA}${SIDLOWER}-zone${i} --tags SAPSID=${SAPSID} SAPHOSTTYPE=${SAPHOSTTYPE} IMPORTANCE=Sandbox >>$LOGFILE 2>&1
...
create_hana_vm () {
    ...
    az vm create --name $VMNAME --resource-group $RGNAME  --os-disk-name ${VMNAME}-osdisk --os-disk-size-gb 63 --storage-sku StandardSSD_LRS --size $VMTYPE --vnet-name $VNETNAME  --location $AZLOC --accelerated-networking true --public-ip-address '' --private-ip-address ${DBSUBNET}.${ip} --image $VMIMAGE --admin-username=$ADMINUSR --ssh-key-value=$ADMINUSRSSH --subnet=${VNETNAME}-db --nsg nsg-${AZLOCTLA}sap-${SIDLOWER}-db --zone $i --ppg ppg-${AZLOCTLA}${SIDLOWER}-zone${i} --tags SAPSID=${SAPSID} SAPHOSTTYPE=HANA HANASID=${HANASID} IMPORTANCE=Sandbox >>$LOGFILE 2>&1   
    az vm disk attach --resource-group $RGNAME --vm-name $VMNAME --name ${VMNAME}-datadisk1 --new --lun 0 --sku StandardSSD_LRS --size 63 >>$LOGFILE 2>&1
...
if [[ $USESPOTINSTANCES -eq "true" ]];" ; fi
